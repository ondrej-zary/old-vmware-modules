...
extern uint64 HashFunc_HashBytes(uint8 *key,  uint32 nBytes);
extern uint64 HashFunc_HashMoreBytes(uint8 *key, uint32 nBytes, 
                                     uint64 initialValue);
extern uint64 HashFunc_HashBytesSlow(uint8 *key, uint32 nBytes);
extern uint64 HashFunc_HashQuads(uint64 *key, uint32 nQuads);
extern uint64 HashFunc_HashPage(void *addr);
...
static INLINE ub8 hash(register ub1 *k,       /* the key */
...
--------------------------------------------------------------------
*/
static INLINE ub8 hash2(register ub8 *k,       /* the key */
                        register ub4 length,   /* the length of the key */
                        register ub8 level)    /* the previous hash, or an arbitrary value */
{
  register ub8 a,b,c;
...
static INLINE ub8 hash3(register ub1 *k,       /* the key */
                        register ub4 length,   /* the length of the key */
                        register ub8 level)    /* the previous hash, or an arbitrary value */
{
  register ub8 a,b,c;
