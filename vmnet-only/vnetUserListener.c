...
typedef struct VNetUserListener {
   ...
   struct semaphore lock;                  /* listener lock */
                             ...
                             VNet_EventHeader *e) // IN: an event
{
   ...
   t = kmalloc(EVENT_NODE_HEADER_SIZE + e->size, GFP_USER);
   if (t == NULL) {
